import { Rethinkdbdash, RDBVal, RDBTerm } from './classes'

let instantFuncs = 'limit orderBy add eqJoin run toArray filter get getAll reduce coerceTo table db'.split(' ')
...
export default function start(r) {
  ...
  function execute(conn) {
    let rethinkdbdash = conn instanceof Rethinkdbdash ? conn : undefined
    return reql => {
